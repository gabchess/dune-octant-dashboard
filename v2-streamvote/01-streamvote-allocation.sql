-- Octant v2 - StreamVote Project Allocation
-- Shows current allocation of voting units across all 17 StreamVote projects
-- Data source: Superfluid GDA MemberUnitsUpdated events
...
SELECT
  ...
  END AS project_name,
  new_units,
  CAST(new_units AS DOUBLE) * 100.0 / SUM(CAST(new_units AS DOUBLE)) OVER () as pct_share
FROM latest_units
WHERE rn = 1
  AND new_units > 0
